...
class ProductCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->cover) {
            ...
            $cover_name = Str::slug($request->name) . date('-Y-m-d-h-i-s-') . rand(999, 9999) . '.' . $extention;
            $cover_name = 'uploads/custom-images/' . $cover_name;
            Image::make($request->cover)
                ->save(public_path() . '/' . $cover_name);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->cover) {
            $old_logo = $category->cover;
            $extention = $request->cover->getClientOriginalExtension();
            $cover_name = Str::slug($request->cover) . date('-Y-m-d-h-i-s-') . rand(999, 9999) . '.' . $extention;
            $cover_name = 'uploads/custom-images/' . $cover_name;
            Image::make($request->cover)
                ->save(public_path() . '/' . $cover_name);
            $category->cover = $cover_name;
